...
public class PlayerMovement2 : MonoBehaviour
{
    ...
    private void Awake() // Use Awake for component setup
    {
        ...
        crouchAction = playerInputActions.Player.Crouch;
        slideAction = playerInputActions.Player.Slide;
    }
    ...
    private void MyInput()
    {
        ...
        if (slideAction.triggered && !isSliding && !startedSlide)
        {
            startedSlide = true;
            isSliding = true;
        ...
        if (slideAction.IsPressed() && (horizontalInput != 0f || verticalInput != 0f) && grounded && readyToJump && isSliding)
        {
            slidingScript.SlideMovement();
        }

        if ((slideAction.WasReleasedThisFrame() && isSliding) || (!grounded && isSliding))
        {
            isSliding = false;
